...
const ContractDetails: React.FC<ContractDetailsProps> = ({ contractId }) => {
  ...
  const { contract, isLoadingContract, updateContract, isUpdatingContract } =
    useContracts(parseInt(contractId));
  const {
    documents,
    isLoading: isLoadingDocuments,
    uploadDocument,
  } = useDocuments(parseInt(contractId));
  ...
  const canPerformAction = (action: string): boolean => {
    ...
    switch (action) {
      case "fund":
        return (
          contract.status === ContractStatus.AWAITING_FUNDS &&
      ...
      case "confirmReceipt":
        return (
          contract.status === ContractStatus.GOODS_SHIPPED &&
          userParty.role === PartyRole.IMPORTER
        );
      case "raiseDispute":
        return (
          [ContractStatus.FUNDED, ContractStatus.GOODS_SHIPPED].includes(
            contract.status
          ) &&
          (userParty.role === PartyRole.IMPORTER ||
            userParty.role === PartyRole.EXPORTER)
        ...
        <div className="flex flex-col md:flex-row justify-between md:items-center mt-2">
          ...
          <Badge
            ...
            className={`bg-${getStatusColor(
              ...
              contract.status
            )}-800 border-${getStatusColor(contract.status)}-200 mt-2 md:mt-0`}
          >
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                    ...
                    {contract.tradeTerms?.deliveryDeadline && (
                      <div className="flex items-center">
                        ...
                        <span className="text-sm font-medium">
                          {formatDate(contract.tradeTerms.deliveryDeadline)}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="space-y-2">
                      {Object.entries(contract.milestones || {}).map(
                        ([key, date]) =>
                          date && (
                ...
                <div className="space-y-3">
                  {contract.status === ContractStatus.DRAFT && (
                    <Button
                      className="w-full"
                      onClick={() =>
                        updateContractStatus(ContractStatus.AWAITING_FUNDS)
                      }
                    >
                      Publish Contract
                    </Button>
                  ...
                  {canPerformAction("fund") && (
                    <Button
                      className="w-full"
                      onClick={() =>
                        updateContractStatus(ContractStatus.FUNDED)
                      }
                    >
                      Fund Escrow
                    </Button>
                  ...
                  {canPerformAction("confirmShipment") && (
                    <Button
                      className="w-full"
                      onClick={() =>
                        updateContractStatus(ContractStatus.GOODS_SHIPPED)
                      }
                    >
                      Confirm Shipment
                    </Button>
                  ...
                  {canPerformAction("confirmReceipt") && (
                    <Button
                      className="w-full"
                      onClick={() =>
                        updateContractStatus(ContractStatus.GOODS_RECEIVED)
                      }
                    >
                      Confirm Receipt
                    </Button>
                  ...
                  {contract.status === ContractStatus.GOODS_RECEIVED && (
                    <Button
                      className="w-full"
                      onClick={() =>
                        updateContractStatus(ContractStatus.COMPLETED)
                      }
                    >
                      Complete Contract
                    </Button>
                  ...
                  {canPerformAction("raiseDispute") && (
                    <Button
                      ...
                      className="w-full"
                      onClick={() =>
                        updateContractStatus(ContractStatus.DISPUTED)
                      }
                    >
                      Raise Dispute
                    </Button>
                  ...
                  >
                    ...
                    <DialogContent>
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          Add a document to this contract. Documents are
                  ...
                  {[
                    ...
                    ContractStatus.AWAITING_FUNDS,
                  ].includes(contract.status) && (
                    <Button
                      variant="outline"
                      className="w-full text-red-500 hover:text-red-700 hover:bg-red-50"
                      onClick={() =>
                        updateContractStatus(ContractStatus.CANCELLED)
                      }
                    >
                      Cancel Contract
                    </Button>
                ...
                {contract.parties.map((party, index) => (
                  ...
                  >
                    <div className="flex flex-col md:flex-row justify-between items-start">
                      <div>
                        <div className="flex items-center">
                          <Badge className="mr-2">{party.role}</Badge>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="flex items-center mt-1">
                        ...
                        <span className="font-medium">
                          {contract.tradeTerms.amount}{" "}
                          {contract.tradeTerms.currency}
                        </span>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="mt-1 font-medium">
                        {contract.tradeTerms.paymentTerms}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="mt-1 font-medium">
                        {contract.tradeTerms.inspectionPeriod} days
                      </div>
                    </div>
                  </div>
                </div>
