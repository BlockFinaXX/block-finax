...
const RiskTrendsChart: React.FC<RiskTrendsChartProps> = ({ trends }) => {
  ...
  const prepareChartData = () => {
    ...
    const filteredDates = sortedDates.filter(
      (date) => new Date(date).getTime() >= cutoffDate.getTime()
    );
  ...
  const getTrendDirection = (
    ...
    const values = [...trend.values].sort(
  ...
  const calculateAverageRisk = () => {
    if (selectedTrends.length === 0)
      return { avg: 0, direction: "stable" as const };
    ...
    const avg =
      latestValues.reduce((sum, val) => sum + val, 0) / latestValues.length;
          ...
          <div className="bg-white p-4 rounded-lg border shadow-sm">
              ...
              {selectedTrends.map((trend) => (
                <Badge
                  ...
                  style={{
                    ...
                    borderColor: getCategoryColor(trend.category) + "40",
                  }}
                >
                  {trend.category}
            ...
            <div className="mt-4 flex justify-between items-center">
              <div className="text-sm text-gray-500">
                <span className="font-medium">Note:</span> Dashed lines
                represent AI-generated predictions with{" "}
                {(trends[0]?.forecast[0]?.confidence || 0.8) * 100}% confidence.
              </div>
