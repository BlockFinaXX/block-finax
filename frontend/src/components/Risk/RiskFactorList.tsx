...
const RiskFactorList: React.FC<RiskFactorListProps> = ({ riskFactors }) => {
  ...
  const groupedByCategory = Object.values(RiskCategory)
    .map((category) => {
      ...
      const avgValue =
        categoryFactors.reduce((sum, factor) => sum + factor.value, 0) /
        categoryFactors.length;
      const maxValue = Math.max(
        ...categoryFactors.map((factor) => factor.value)
      );

      const criticalCount = categoryFactors.filter(
        (factor) => factor.level === RiskLevel.CRITICAL
      ).length;
      const highCount = categoryFactors.filter(
        (factor) => factor.level === RiskLevel.HIGH
      ).length;
      const mediumCount = categoryFactors.filter(
        (factor) => factor.level === RiskLevel.MEDIUM
      ).length;
      const lowCount = categoryFactors.filter(
        (factor) => factor.level === RiskLevel.LOW
      ).length;

      const increasingCount = categoryFactors.filter(
        (factor) => factor.trend === "increasing"
      ).length;
      const decreasingCount = categoryFactors.filter(
        (factor) => factor.trend === "decreasing"
      ).length;
      const stableCount = categoryFactors.filter(
        (factor) => factor.trend === "stable"
      ).length;
        ...
        <Card className="shadow-md">
          ...
          <CardContent>
            <div className="space-y-4">
                    ...
                    <div key={group.category} className="space-y-1">
                      <div className="flex justify-between items-center">
                        <div className="flex items-center">
                          <div
                            className={`p-1.5 rounded-full bg-${levelColor}-50 text-${levelColor}-500 mr-2`}
                          >
                      ...
                      <div className="flex text-xs text-gray-500 justify-between">
                        ...
                        <div>Avg: {group.avgValue.toFixed(0)}%</div>
                      </div>
              ...
              {groupedByCategory.filter((group) => group.count > 0).length ===
                0 && (
              ...
              {riskFactors
                ...
                .map((factor) => {
                  ...
                  return (
                    <Card
                      key={factor.id}
                      className={`shadow-sm border-l-2 border-l-${color}-500`}
                    >
                      <CardContent className="p-3">
                        <div className="flex items-start gap-3">
                          <div
                            className={`p-1.5 rounded-full bg-${color}-50 text-${color}-500 mt-0.5 flex-shrink-0`}
                          >
                          ...
                          <div className="space-y-1 flex-1">
                            ...
                            <div className="flex text-xs text-gray-500 justify-between pt-1">
                              <div>
                                {factor.impactDescription.substring(0, 60)}...
                              </div>
                            </div>
                          </div>
                        </div>
          ...
          <CardContent className="border-b pb-4">
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              ...
              <div className="space-y-1">
                ...
                <Select
                  value={filters.trend}
                  onValueChange={(value) =>
                    setFilters({ ...filters, trend: value })
                  }
                >
                  <SelectTrigger>
            ...
            <div className="flex justify-between mt-3">
              <div className="text-xs text-gray-500">
                Showing {filteredFactors.length} of {riskFactors.length} risk
                factors
              </div>
              <Button
                variant="ghost"
          ...
          {sortedFactors.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-lg border shadow-sm">
              ...
              <p className="text-gray-500 mt-2 max-w-md mx-auto">
                No risk factors match your current filter criteria. Try
                adjusting your filters or generate new risk assessments.
              </p>
                        ...
                        <div className="flex-1 space-y-2">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            >
                              ...
                              {factor.trend.charAt(0).toUpperCase() +
                                factor.trend.slice(1)}
                            </Badge>
                          </div>
